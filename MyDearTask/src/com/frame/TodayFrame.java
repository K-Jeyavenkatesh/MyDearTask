...
public class TodayFrame implements ActionListener{
	
	JPanel mainpanel;
	JPanel[] pp;
	private String user;
	
	public JScrollPane TodayFrame(String user) {
		...
		ArrayList<ArrayList<String>> arr = new FetchDataToday().FetchDataToday(user);
	...
	public void actionPerformed(ActionEvent e) { 
		...
		if(button_type[0].equals("COMPLETED")) {
			...
			int change_com = new FetchDataToday().changeToday(pp[t].getName());
			if(change_com == 1) {
			...
			int change_post = new FetchDataToday().postponeToday(pp[t].getName());
			if(change_post == 1) {
