...
import com.database.FetchDataToday;

public class PendingFrame implements ActionListener{
	
	JPanel mainpanel;
	JPanel[] pp;
	private String user;
	
	public JScrollPane PendingFrame(String user) {
		...
		ArrayList<ArrayList<String>> arr = new FetchDataPending().FetchDataPending(user);
	...
	public void actionPerformed(ActionEvent e) { 
		...
		if(button_type[0].equals("COMPLETED")) {
			...
			int change_com = new FetchDataPending().changePending(pp[t].getName());
			if(change_com == 1) {
			...
			int change_post = new FetchDataPending().postponePending(pp[t].getName());
			if(change_post == 1) {
